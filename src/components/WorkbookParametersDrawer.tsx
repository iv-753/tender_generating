...
export default function WorkbookParametersDrawer({ open, zhujiang, parameters, overrides, loading, error, focusKey, missingKeys, onClose, onSave }: Props) {
  ...
  useEffect(() => { if(open) {setQuery(focusKey ? parameters.find(item=>item.key===focusKey)?.label??'' : '');setGroup(!focusKey && parameters.some(item=>item.group==='珠江·项目及配置') ? '珠江·项目及配置' : undefined);setMissingOnly(false);setPage(1);} },[open,focusKey,parameters]);
  ...
  const filtered = visible.filter((item) => (!group || item.group === group) && (!missingOnly || (missingKeys ? missingKeys.includes(item.key) : item.defaultValue===null)) && item.label.includes(query.trim()));
  ...
  return <Drawer title="项目测算参数" open={open} onClose={onClose} size={960} destroyOnHidden footer={<Space style={{ width: '100%', justifyContent: 'space-between' }}><Typography.Text type="secondary">已手动设置 {modifiedCount} 项；恢复默认后按所选档次重新计算。</Typography.Text><Space><Button onClick={onClose}>取消</Button><Button type="primary" disabled={loading || !parameters.length || Object.values(invalid).some(Boolean)} onClick={() => onSave(draft)}>应用参数</Button></Space></Space>}>
    <Alert type="info" showIcon title={zhujiang ? "珠江标准与项目参数" : "原表完整计算口径"} description={zhujiang ? "固定频次按所选珠江档次自动带入，人员配比可在原区间基础上调整。设备、服务范围、工资及实际工时由项目确认，并非珠江统一数值。没有的设施填0，未知留空；补充范围并应用后，系统会列出后续需要填写的作业计划。" : "按广东区级单价计算。保留原表工时、人员取整及附加比例；替休采用原公式5.2（原表最低档文字写7，待核实）。模板数量和历史单价可在这里调整。"} style={{ marginBottom: 16 }} />
    {error && <Alert type="error" showIcon title={error} style={{ marginBottom: 16 }} />}
    <Space wrap style={{ marginBottom: 16 }}><Select aria-label="参数分类" placeholder="全部分类" allowClear style={{ width: 180 }} value={group} options={groups.map((value) => ({ value, label: value }))} onChange={(value) => { setGroup(value); setPage(1); }} /><Input aria-label="搜索计算参数" placeholder="搜索动作、工资、面积等" allowClear value={query} onChange={(event) => { setQuery(event.target.value); setGroup(undefined); setPage(1); }} />{zhujiang && <Checkbox checked={missingOnly} onChange={e=>{setMissingOnly(e.target.checked);setGroup(undefined);setPage(1);}}>只看待补数据</Checkbox>}<Button disabled={!modifiedCount} onClick={() => { setDraft(current => Object.fromEntries(Object.entries(current).filter(([key]) => !parameters.some(item => item.key === key)))); setInvalid({}); }}>项目参数恢复默认</Button></Space>
