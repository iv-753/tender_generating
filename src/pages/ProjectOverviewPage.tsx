...
export default function ProjectOverviewPage({ onNavigate }: { onNavigate: (path: ProjectWorkspacePath) => void }) {
  ...
  const { result } = record;
  const stages = [
    { title: '项目资料', status: '已归档' }, { title: '智能测算', status: result.calculationModel === 'zhujiang-v1' ? '待补全核实' : '已完成' },
