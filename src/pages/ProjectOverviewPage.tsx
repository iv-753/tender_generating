...
export default function ProjectOverviewPage({ onNavigate }: { onNavigate: (path: ProjectWorkspacePath) => void }) {
  ...
  const { result } = record;
  const stages = [
    { title: '项目资料', status: '已归档' }, { title: '智能测算', status: result.calculationModel === 'zhujiang-v1' ? result.standard?.complete ? '已量化服务成本' : '待补项目数据' : '已完成' },
    { title: '路演PPT', status: record.presentation ? '已生成' : '待生成' }, { title: '投标标书', status: record.bidDocument ? '已生成' : '待生成' },
  ];
  return <main className="workspace-page">
    <div className="result-heading blueprint-rule"><div><Typography.Title level={2}>{result.project.projectName}</Typography.Title><Typography.Paragraph type="secondary">{formatProjectLocation(result.project)} · {gradeLabel(result.project.serviceGrade, result.calculationModel)} · {result.calculationModel === 'zhujiang-v1' ? `珠江标准 · ${result.budgetBasis === 'workload' ? '服务工时口径' : '珠江配比口径'}` : result.calculationModel === 'workbook-v3' ? '原表完整算法 · 区级单价' : COST_BAND_LABELS[result.project.costBand]}</Typography.Paragraph></div><Button onClick={() => onNavigate('/project/result')}>查看测算结果 <ArrowRightOutlined /></Button></div>
    ...
    <section className="metrics-grid overview-metrics"><Card><Statistic title="住宅收费面积" value={result.project.residentialChargeArea} formatter={(value) => number.format(Number(value))} suffix="㎡" /></Card><Card><Statistic title="服务动作" value={result.totalActionCount} suffix="项" /></Card><Card><Statistic title={result.calculationModel === 'zhujiang-v1' ? '配置及分摊人数' : '配置总人数'} value={displayStaffingCount(result.totalHeadcount, result.calculationModel)} suffix="人" /></Card><Card className="cost-card"><Statistic title={result.calculationModel === 'zhujiang-v1' ? result.standard?.complete ? '年度服务成本' : '费用小计（待补数据）' : '年成本'} value={result.annualCost} formatter={(value) => currency.format(Number(value))} /></Card></section>
