...
export default function ProjectResultPage({ onNavigate }: ProjectResultPageProps) {
  ...
  const columns = [
    ...
    ...(zhujiang ? [{ title: '规则来源', key: 'standard', width: 160, render: (_: unknown, item: ServiceActionResult) => <Tooltip title={<>{item.standardSource}<br />{item.standardText}<br />{item.standardNote}</>}><Tag color={item.standardStatus === 'pending' ? 'orange' : item.standardStatus === 'excluded' ? 'default' : item.standardStatus === 'manual' ? 'gold' : 'blue'}>{item.standardStatus === 'pending' ? '待确认 · 未计价' : item.standardStatus === 'excluded' ? '不适用' : item.standardStatus === 'reference' ? '参考参数' : item.standardStatus === 'manual' ? '项目调整' : '珠江规则'}</Tag></Tooltip> }] : []),
  ...
  return (
    <main className="workspace-page">
      ...
      {restored && <Alert type="info" showIcon title={zhujiang ? "珠江测算小计 · 尚未形成完整报价" : "按原表计算的服务预算"} description={zhujiang ? `两种口径独立计算，不取高、不相加。当前有${result.standard?.pendingActionCount ?? 0}项动作待确认，未计入工时或专业作业费用；不代表这些服务可以省略。未映射动作须确认频次，设备数量须按项目填写。单价是小计折算值，尚未扣除车库等收入，不能作为完整物业费或盈亏平衡价。` : "保留原表的岗位折算、取整和附加比例。服务成本单价不代表完整经营盈亏平衡价；模板数量与历史价格请按项目实际核实。"} style={{ marginBottom: 16 }} />}
      {zhujiang && result.standard?.pendingActionCount === undefined && <Alert type="warning" showIcon title="这是修正前的历史结果，请打开调整计算参数并应用，重新测算。" style={{ marginBottom: 16 }} />}
      ...
      {zhujiang && result.budgetComparison && <Card title="两种预算口径对比" style={{ marginBottom: 16 }}>
        ...
        <Table rowKey="basis" size="small" pagination={false} scroll={{ x: 640 }} dataSource={(['standard', 'workload'] as const).map(basis => ({ basis, ...result.budgetComparison![basis] }))} columns={[
          ...
          { title: '配置人数', key: 'headcount', render: (_, row) => `${displayStaffingCount(row.headcount)} 人` },
      ...
      <section className="metrics-grid">
        ...
        <Card className="cost-card"><Statistic title={explainedTitle(zhujiang ? '年度费用小计（待补全）' : '项目年度用工预算', zhujiang ? '采用当前所选预算口径，包含管理成本；两套预算不相加。' : restored ? '按原表各分类的工时、人员折算、取整和附加比例汇总。' : '汇总工作量后按完整岗位人数向上取整，小幅调整时预算可能暂时不变。')} value={result.annualCost} formatter={(value) => wholeNumber.format(Number(value))} prefix="¥" /></Card>
      ...
      {restored && <WorkbookParametersDrawer zhujiang={zhujiang} open={parametersOpen} parameters={workbookInputs} overrides={savedResult.project.workbookOverrides ?? EMPTY_WORKBOOK_OVERRIDES} loading={parametersLoading} error={parametersError} onClose={() => { if (!parametersLoading) setParametersOpen(false); }} onSave={applyParameters} />}
      <Modal
