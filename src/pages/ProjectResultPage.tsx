import { ArrowLeftOutlined, FilePptOutlined, FileTextOutlined, LoadingOutlined, SearchOutlined } from '@ant-design/icons';
...
const bidGenerationStages = [
  { key: 'validating', title: '校验测算数据', description: '确认项目资料与服务动作完整' },
  { key: 'preparing', title: '整理服务方案', description: '汇总项目指标与服务标准' },
  { key: 'binding', title: '套用标书模板', description: '将项目内容写入标准Word模板' },
  { key: 'exporting', title: '导出标书文件', description: '生成可继续编辑的投标文件' },
] as const;
...
export default function ProjectResultPage({ onNavigate }: ProjectResultPageProps) {
  ...
  const [bidGenerationOpen, setBidGenerationOpen] = useState(false);
  const [bidGeneration, setBidGeneration] = useState<GenerationJob>({ status: 'idle', stage: 'validating' });
  ...
  const generateBidDocument = async () => {
    setBidGenerationOpen(true);
    setBidGeneration({ status: 'running', stage: 'validating' });
    try {
      const createdResponse = await fetch('/api/bid/jobs', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(result),
      });
      const created = await createdResponse.json() as GenerationJob;
      if (!createdResponse.ok || !created.jobId) throw new Error(created.error || '无法开始生成标书');
      setBidGeneration(created);
      ...
      while (latest.status === 'running') {
        ...
        latest = await response.json() as GenerationJob;
        if (!response.ok) throw new Error(latest.error || '无法获取生成进度');
        setBidGeneration(latest);
        if (latest.status === 'running') await new Promise((resolve) => window.setTimeout(resolve, 500));
      }
    } catch (error) {
      setBidGeneration((current) => ({ ...current, status: 'error', error: error instanceof Error ? error.message : '标书生成失败' }));
    }
  };
  ...
  const currentBidStage = bidGeneration.status === 'complete'
    ? bidGenerationStages.length
    : Math.max(0, bidGenerationStages.findIndex((item) => item.key === bidGeneration.stage));
  const bidProgress = bidGeneration.status === 'complete' ? 100 : currentBidStage * 25;

  return (
    <main className="workspace-page">
      <div className="result-heading blueprint-rule"><div><Typography.Text className="eyebrow">CALCULATION RESULT</Typography.Text><Typography.Title level={2}>{result.project.projectName}</Typography.Title><Typography.Paragraph type="secondary">{result.project.region} · {gradeLabel(result.project.serviceGrade)} · {COST_BAND_LABELS[result.project.costBand]}</Typography.Paragraph></div><Space wrap><Button icon={<ArrowLeftOutlined />} onClick={onNavigate}>返回修改</Button><Button icon={<FileTextOutlined />} loading={bidGeneration.status === 'running'} onClick={generateBidDocument}>生成投标标书</Button><Button type="primary" icon={<FilePptOutlined />} loading={generation.status === 'running'} onClick={generatePresentation}>生成路演PPT</Button></Space></div>
      ...
      <Modal
        className="generation-modal"
        open={bidGenerationOpen}
        title={bidGeneration.status === 'complete' ? undefined : bidGeneration.status === 'error' ? '标书生成未完成' : '正在生成投标标书'}
        width={620}
        centered
        closable={bidGeneration.status !== 'running'}
        mask={{ closable: bidGeneration.status !== 'running' }}
        onCancel={() => setBidGenerationOpen(false)}
        footer={bidGeneration.status === 'complete' ? (
          <Space><Button onClick={() => setBidGenerationOpen(false)}>返回测算结果</Button><Button type="primary" icon={<FileTextOutlined />} href={bidGeneration.downloadUrl} download={bidGeneration.fileName}>下载标书</Button></Space>
        ) : bidGeneration.status === 'error' ? (
          <Space><Button onClick={() => setBidGenerationOpen(false)}>关闭</Button><Button type="primary" onClick={generateBidDocument}>重新生成</Button></Space>
        ) : null}
      >
        {bidGeneration.status === 'complete' ? (
          <Result
            status="success"
            title="投标标书已生成"
            subTitle={<span className="generated-file"><strong>{bidGeneration.fileName}</strong><span>Word 文档</span></span>}
          />
        ) : bidGeneration.status === 'error' ? (
          <Result status="error" title="生成失败" subTitle={bidGeneration.error || '请检查后重新生成'} />
        ) : (
          <div className="generation-progress">
            <Typography.Paragraph type="secondary">{result.project.projectName}</Typography.Paragraph>
            <Progress percent={bidProgress} showInfo={false} strokeColor="#2f7d73" railColor="#e4ecef" />
            <Steps
              orientation="vertical"
              current={currentBidStage}
              items={bidGenerationStages.map((item, index) => ({
                title: item.title,
                content: item.description,
                icon: index === currentBidStage ? <LoadingOutlined /> : undefined,
              }))}
            />
          </div>
        )}
